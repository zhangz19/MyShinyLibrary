---
title: "Source code for Simon's 2-stage design with operating characteristics"
author: 
...
### Simon's 2-stage with OC curves
```{r, eval=TRUE, tidy=TRUE}
input <- list(pu=.2, pa=.4, e1=.05, pwr=.9)  #an example input for the design
...
# Simon 2-stage
print(tmp <- clinfun::ph2simon(pu=pu, pa=pa, ep1=e1, ep2=e2, nmax=100))
...
knitr::kable(dsn, row.names=TRUE, caption = "\\label{tab:t1}Designs.", digits=3)
```
<a href="#top">Back to top</a>

### OC curve plot
```{r, eval=TRUE, tidy=TRUE, fig.width=7.3, fig.height=4, fig.cap="\\label{fig:f1}OC curve", out.width='100%'}
vec <- seq(0, 0.7, len=1e3)
pet <- pbinom(design[['r1']], design[['n1']], vec)
pwr <- 1-sapply(vec, p_rej)
ego <- sapply(vec, p_go)
en <- with(design, 1-(n1+(1-pet)*(n-n1))/n)
ego1s <- 1 - pbinom(floor(n0*pa), n0, vec)

par(mar=c(2,2.4,4,.3)+.2, mgp=c(1.3,.3,0), tck=-0.01, cex.axis=1, cex.lab=1.1, 
    cex.main=1, las=1, font.lab=2) 
...
plot(1, type='n', xlim=range(vec), ylim=c(0,1), 
     ...
     main=paste0("Operating Characteristics of Two-Stage Optimal Design\nH0: p<=",round(pu,2),
                 " vs H1: p>=",round(pa,2), "\nStop if responders<=",design[['r1']],
                 " out of ",design[['n1']],", and <=",design[['r']],
                 " out of ",design[['n']], ", ESoE Go if >=",pa))
par(yaxt='s')
ylabs <- c(0,.05,.25,.5,.75,.9,1); axis(side=2, at=ylabs, labels=paste0(100*ylabs,'%'))
...
legend(x=usr[2]-0.36*xran, y=usr[3]+.52*yran, 
       legend=c('Early Termination','Reduce sample %','Continuation','ESoE Go',
        'ESoE Go (1-stage)'), col=cols[c(1,4,2,3,3)], lty=c(1,1,1,1,2), lwd=c(2,4,2,2,2), 
       bg='white', cex=1, bty='n', text.font=2, text.width=4)
```
<a href="#top">Back to top</a>
